...
postscript("../Figs/fig1.ps", horiz=FALSE, height=2.5, width=6.5)
...
arrows(first+4.5*mult,cury+10,first+4.5*mult,cury+3,length=0.1,xpd=TRUE)
...
for(i in 1:41)
  text(first+(i-1+5)*mult,cury,substr(mt598[2],i+41,i+41),
       cex=0.8,adj=c(0.5,0.5))
arrows(first+35.5*mult,cury+10,first+35.5*mult,cury+3,length=0.1)
...
for(i in 1:5)
  text(first+(i-1+41+5)*mult,cury,substr(mt598[3],i,i),
       cex=0.8,adj=c(0.5,0.5))
arrows(first+43.5*mult,cury+10,first+43.5*mult,cury+3,length=0.1)
